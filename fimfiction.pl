...
sub uri_parse { 
    my ($url) = @_; 
    # Check for a post...
    if ($url =~  /(?:https?:\/\/)?(?:[0-9A-Z-]*\.)?(?:fimfiction\.net)\/story\/(\d+)/ig) {
        return {url=>"http://www.fimfiction.net/api/story.php?story=$1",service=>"FimFiction"};
    }
    return 0; 
} 
						...
						if ($story->{categories}) {
							...
							foreach my $category (keys %$categories) {
								if ($categories->{$category}) {
									unshift(@tags,$category);
								}
							}
							if (scalar @tags > 0) {
								$result_string .= ", flagged: " . join(", ",@tags);
							}
						}
					}
